...
module  efield_ipe
  ...
  !   May 2026 Aaron Ridley - Added MILE, reformatted to F90
  !---------------------------------------------------------------------------- 
  ...
  public :: &
       efield_init_ipe, & ! interface routine                     
       get_efield_ipe ! interface routine
  public :: &
       ed1, & ! zonal electric field Ed1  [V/m] 
  ...
  public :: &
       ALAMN, ALAMX, ALAMR, &
       STPD, STP2, CSTP, SSTP, &
       CX, ST, CT, AM, EPOCH, TH0, PH0, DIPOLE
  ...
  ! setting weimer2005 model
  character*4, parameter ::  my_model = 'epot' 
  ...
  ! latitude index for interpolation of 
  ...
  real , parameter :: EPOCH=1980., TH0=11.19, PH0=-70.76, DIPOLE=.30574

  !---------------------------------------------------------------------- 
  ! high_latitude boundary
  !---------------------------------------------------------------------- 
  ! max e-field for high latitude boundary location [V/m]
  real, parameter :: ef_max  = 0.015  
  ! shift of highlat_bnd to 54 deg
  real, parameter :: lat_sft = 54.
...
contains
  ...
  subroutine efield_init_ipe(rc)
    ...
    character(len=*), parameter :: &
         efield_lflux_file = dataDir // 'global_idea_coeff_lflux.dat', &
         efield_hflux_file = dataDir // 'global_idea_coeff_hflux.dat'
    ...
    call read_acoef( &
         efield_lflux_file, &
         efield_hflux_file, &
         rc=lrc)    ! read in A_klnm for given S_aM
    if (ipe_error_check(lrc, msg="call to read_acoef failed", rc=rc)) &
         return
    call index_quiet(lrc)  ! set up index for f_m(mlt),f_l(UT),f_-k(d)
    if (ipe_error_check(lrc, msg="call to index_quiet failed", rc=rc)) &
         return
    ...
    if (trim(model) == 'epot') then
       call read_potential(dataDir // 'global_idea_coeff_W05scEpot.dat',rc=lrc)
       if (ipe_error_check(lrc, msg="failed to read potential (epot)", rc=rc)) &
            return
    else
       call read_potential(dataDir // 'global_idea_coeff_W05scBpot.dat',rc=lrc)
       if (ipe_error_check(lrc, msg="failed to read potential (bpot)", rc=rc)) &
            return
    endif

    call read_schatable(dataDir // 'global_idea_coeff_W05SCHAtable.dat',rc=lrc)
    if (ipe_error_check(lrc, msg="failed to read schatable", rc=rc)) &
         return

    call read_bndy(dataDir // 'global_idea_coeff_W05scBndy.dat',rc=lrc)
    if (ipe_error_check(lrc, msg="failed to read boundary", rc=rc)) return
  ...
  subroutine get_efield_ipe(i_rank,rc)
    ...
    if (ipe_error_check(lrc,msg="call to GlobalElPotential failed", rc=rc)) &
         return
  ...
  subroutine GlobalElPotential(rc)
    ...
    real :: mlat2d(nmlon+1, nmlat+1), mlt2d(nmlon+1, nmlat+1)
    real :: pot2d(nmlon+1, nmlat+1)
   ...
   if (useMile) then
      ...
      if (iMileVerbose > 0) then
         write(*,*) "-> MILE: This is the empirical electric potential"
         write(*,*) "-> MILE: getting potential in efield_ipe"
         write(*,*) " --> nMlts, nLats : ", nmlon, nmlat
      endif
      ! We really don't know how many different times the IE library is going to
      ! be needed in each time-step, so set the grid again:
      call IEModel_%nMlts(nmlon + 1)
      call IEModel_%nLats(nmlat + 1)
      do ilat = 0,nmlat      ! Calculate only for one magn. hemisphere
         mlat_90 = 90. - ylatm(ilat) ! mag. latitude
         do ilon = 0,nmlon
            mlat2d(ilon, ilat) = mlat_90
            mlt2d(ilon, ilat) = ylonm(ilon)*deg2mlt
         enddo
      enddo
      ...
      do ilat = 0,nmlat_wei  ! Calculate only for one magn. hemisphere
         pot_highlat(0:nmlon, ilat) = pot2d(1:nmlon+1, ilat)
         ! Force north/south symmetry from some reason:
         pot_highlat(0:nmlon, nmlat - ilat) = pot2d(1:nmlon+1, ilat)
      enddo

      if (iMileVerbose > 0) then
         write(*,*) ' --> cpcp : ', maxval(pot2d) - minval(pot2d)
      endif

#else
      write(*,*) "Somehow useMile is TRUE, but code is not configured!!!"
      ...
      do ilat = 0,nmlat_wei  ! Calculate only for one magn. hemisphere
         ...
         do ilon = 0,nmlon
            ...
            if (ipe_error_check(lrc, msg="call to EpotVal_new failed", rc=rc)) &
                 return
            pot = 1000.*pot
   ...
   !-----------------------------------------------------------------------
   call interp_poten( &
        pot_highlats, pot_highlat, pot_midlat,  ihlat_bnd, itrans_width) 
 ...
 subroutine prep_pnm
   !-----------------------------------------------------------------      
   ! Purpose:
   !      constant factors for normalized associated Legendre polynomial P_n^m
   !          Ref.: Richmond J.Atm.Ter.Phys. 1974
   !
   ! Method:
 ...
 subroutine index_quiet(rc)
   ...
   imax = i - 1  
   if (ipe_status_check(imax == ni, msg="index_quiet: imax /= ni", rc=rc)) &
        return
 ...
 subroutine read_acoef (efield_lflux_file, efield_hflux_file, rc)
   ...
   if (ipe_iostatus_check(ios, msg="error opening file "//locfn, rc=rc)) return
   !     if(ios.gt.0) then
   !     write(iulog,*) 
   ...
   if (ipe_iostatus_check(ios, msg="error reading a_lf", rc=rc)) return
   !     if(ios.gt.0) then
   !     write(iulog,*) 
   ...
   if (ipe_iostatus_check(ios, msg="error closing file "//locfn,  rc=rc)) return
   ...
   if (ipe_iostatus_check(ios, msg="error reading a_hf", rc=rc)) return
   !     if(ios.gt.0) then
   !      write(iulog,*) 
 ...
 subroutine set_fkflfs( fk, fl, fs )
   ...
   !	... dummy arguments
   !-----------------------------------------------------------------
   real, intent(out) :: fk(0:2) ! f_-k(day) 
   real, intent(out) :: fl(-2:2) ! f_l(ut)  
   real, intent(out) :: fs(2) ! f_s(f10.7) 
   !------------------------------------------------------------------
   ! local variables
   !-------------------------------------------------------------------
   integer :: lp
...
!      
   do ilon = 0,nmlon  ! long.
      !       sum = 0.
      sum = dot_product( &
           lsg(-nmax_sin+ishf:nmax_sin+ishf), &
...
!CC NCAR MODIFICATION (3/96) CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
  ...
  DO I=1,3   
     DO J=1,3   
        AM(I,J,GSEGEO) = &
             AM(I,1,GEIGEO)*AM(1,J,GSEGEI) +  &
             AM(I,2,GEIGEO)*AM(2,J,GSEGEI) + &
             AM(I,3,GEIGEO)*AM(3,J,GSEGEI)
     ENDDO
  ENDDO
  !         
...
!         
  DO I=1,3   
     DO J=1,3   
        AM(I,J,GSEMAG) = &
             AM(I,1,GEOMAG)*AM(1,J,GSEGEO) + &
             AM(I,2,GEOMAG)*AM(2,J,GSEGEO) + &
             AM(I,3,GEOMAG)*AM(3,J,GSEGEO)
     ENDDO
  ENDDO
  !         
  B32 = AM(3,2,GSEMAG)         
...
!
  ...
  use weimer2005_ipe, only: EpotVal_new
